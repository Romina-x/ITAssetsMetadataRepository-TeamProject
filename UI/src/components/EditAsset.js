...
export const fetchAssetById = async (id) => {
  try {
    const response = await fetch(`http://localhost:3000/asset/view/${id}`); // Assuming your API endpoint is something like this
    if (!response.ok) {
      throw new Error('Failed to fetch asset');
    }
    const data = await response.json();
    return data; // Return the fetched asset data
  } catch (error) {
    throw new Error(`Error fetching asset by ID ${id}: ${error.message}`);
  }
};
      ...
      >
        ...
        <Grid item xs={6}>
          <TextField
            ...
            multiline
            value={assetData ? assetData.title : title}
          />
        </Grid>
        <Grid item xs={6}>
          <TextField
            ...
            multiline
            value={assetData ? assetData.link : link}
